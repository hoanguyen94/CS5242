"""
approach2_idea1_v5_32.py — Improved Approach 2 v5: ConvNeXt-Tiny Architecture (img_size 32)
========================================================================
...
Usage
-----
  python approach2_idea1_v5_32.py --save_dir experiments/idea1_v5_32 --device mps
  python3 approach2_idea1_v5_32.py --save_dir experiments/idea1_v5_32 --device mps --epochs 40
"""
...
def train_improved_finetune_v5(
    ...
    save_dir:      Path  = Path("./experiments/idea1_v5_32"),
    mean=None,
    std=None,
) -> dict:
    ensure_dir(save_dir)
    ...
    num_params_mil = count_params(model) / 1e6
    ...
    # Calculate inference time natively
    t0_inf = time.time()
    num_inf = 0
    with torch.no_grad():
        for x, _ in test_loader:
            ...
            _ = model(x)
            num_inf += x.size(0)
    inf_time_ms = ((time.time() - t0_inf) / max(num_inf, 1)) * 1000.0

    # User requested metrics
    results["Pretrained Models"] = True
    results["Number of parameters (mil)"] = float(num_params_mil)
    results["Test accuracy (%)"] = float(test_acc * 100)
    results["Test loss (Cross Entropy)"] = float(test_loss)
    results["Training Time (seconds)"] = float(results["total_train_time_sec"])
    results["Inference Time per Image (ms)"] = float(inf_time_ms)
    results["GPU"] = device.type in ["cuda", "mps"]
    results["Image Size"] = img_size
...
def _parse_args():
    ...
    p.add_argument("--save_dir",      default="experiments/idea1_v5_32")
