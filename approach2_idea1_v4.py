"""
approach2_idea1_v4.py — Improved Approach 2 v4: Catastrophic Forgetting Fix
========================================================================
Achieving max generalisation on ResNet18 by strictly protecting the ImageNet features.
In v3, unfreezing the backbone with a large learning rate caused immediate
catastrophic forgetting (val_acc tanked from 91.5% to 87%).

Changes over v3:
  [A] Dropped complex 3-phase unfreeze for a simple 2-phase unfreeze (Head -> Full).
  [B] Ultra-protective backbone LR: Down to 0.01x to 0.001x of the head LR, effectively
      treating the backbone as mostly frozen / heavily stabilised.
  [C] Replaced OneCycleLR (which spikes midway) with strict CosineAnnealingLR.
  [D] Replaced manual SWA with Continuous EMA (Exponential Moving Average) which
      robustly survives early-stopping and averages across batches.

Usage
-----
  python approach2_idea1_v4.py --save_dir experiments/idea1_v4 --device mps --epochs 40
"""
...
# ──────────────────────────────────────────────
# 4. Ultra-Protected LR Optimizer
# ──────────────────────────────────────────────
...
def build_optimizer(model: nn.Module, base_lr: float, is_phase2: bool = False,
    ...
    if not is_phase2:
        ...
        # Protective backbone LRs! We don't want a repeat of catastrophic forgetting.
        param_groups = [
            {"params": stem_params,   "lr": base_lr * 0.001},
            {"params": layer1_params, "lr": base_lr * 0.001},
            {"params": layer2_params, "lr": base_lr * 0.002},
            {"params": layer3_params, "lr": base_lr * 0.005},
            {"params": layer4_params, "lr": base_lr * 0.01},
...
def train_improved_finetune_v4(
    ds,
    img_size:      int   = 224,
    epochs:        int   = 40,
    batch_size:    int   = 64,
    ...
    save_dir:      Path  = Path("./experiments/idea1_v4"),
    mean=None,
    std=None,
) -> dict:
    ensure_dir(save_dir)
    ...
    for epoch in range(1, epochs + 1):
        ...
        # ── Train one epoch ────────────────────────────────────────────────
        model.train()
        set_bn_eval(model) # Keep backbone BN frozen
...
def _parse_args():
    p = argparse.ArgumentParser(description="Approach 2 v4: Ultra-low backbone LR & EMA Tracker")
    p.add_argument("--save_dir",      default="experiments/idea1_v4")
    p.add_argument("--epochs",        type=int,   default=40)
    p.add_argument("--batch_size",    type=int,   default=64)
    p.add_argument("--lr",            type=float, default=3e-4) # Base LR for the HEAD
...
if __name__ == "__main__":
    ...
    print(f"[v4] 2-Phase EMA. randaug={args.randaug_mag}  dropout={args.dropout}  wd={args.weight_decay} ")
